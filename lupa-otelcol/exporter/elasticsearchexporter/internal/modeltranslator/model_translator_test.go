...
func TestModelTranslatorWithMili(t *testing.T) {
	...
	actualInternalSpans := TranslateOTLPToInternalSpans(traces, WithMiliSec())
...
func TestModelTranslatorWithDedup(t *testing.T) {
	traces := createOTLPTraces()

	actualInternalSpans := TranslateOTLPToInternalSpans(traces, WithDedupAttributes())
	...
	for i := range actualInternalSpansSlice {
		acAttrs := actualInternalSpansSlice[i].Span.Attributes

		acKeys := make([]string, 0)

		for k := range acAttrs {
			acKeys = append(acKeys, k)
		}
		assert.Contains(t, acKeys, "attribute.value")
	}
}
	...
	for i := 0; i < 2; i++ {
		...
		for j := 0; j < 2; j++ {
			...
			for k := 0; k < 2; k++ {
				...
				span.Attributes().PutStr("battribute", fmt.Sprintf("[Span-%d]battribute", k))
				span.Attributes().PutStr("attribute", fmt.Sprintf("[Span-%d]attribute", k))
				span.Attributes().PutStr("attribute.a", fmt.Sprintf("[Span-%d]attribute", k))
...
func createExpectedInternalSpans(inNano bool) []*internalspanv1.InternalSpan {
	...
	for i := 0; i < 2; i++ {
		...
		spans = append(spans, &internalspanv1.Span{
			...
			Attributes: internalspanv1.Attributes{
				"battribute":  fmt.Sprintf("[Span-%d]battribute", i),
				"attribute":   fmt.Sprintf("[Span-%d]attribute", i),
				"attribute.a": fmt.Sprintf("[Span-%d]attribute", i),
			},
